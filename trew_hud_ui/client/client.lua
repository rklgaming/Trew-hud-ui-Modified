...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(player, false)  then
			...
			elseif (vehicleLights == 1 and vehicleHighlights == 1) or (vehicleLights == 0 and vehicleHighlights == 1) and  GetIsVehicleEngineRunning(vehicle) then
			...
			vehicleInfo['key']['status'] = keyisOn
		else
...
Citizen.CreateThread(function()
	while true do
		...
		if  IsPedInAnyVehicle(player, false) and  GetIsVehicleEngineRunning(vehicle) and GetPedInVehicleSeat(vehicle, -1) == player then	
			keyisOn = true
		else
		...
		if IsPedInAnyVehicle(player, false)  then
			if IsControlJustReleased(0, Keys[Config.vehicle.keys.key]) and (has_value(vehiclesCars, vehicleClass) == true and vehicleClass ~= 8) and GetPedInVehicleSeat(vehicle, -1) == player then
				if keyisOn then
					keyisOn = false
				else
		...
		-- Vehicle Cruiser
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.cruiser]) and GetPedInVehicleSeat(vehicle, -1) == player and (has_value(vehiclesCars, vehicleClass) == true) and  GetIsVehicleEngineRunning(vehicle) then
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalLeft]) and (has_value(vehiclesCars, vehicleClass) == true) and GetPedInVehicleSeat(vehicle, -1) == player and  GetIsVehicleEngineRunning(vehicle) then
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalRight]) and (has_value(vehiclesCars, vehicleClass) == true) and GetPedInVehicleSeat(vehicle, -1) == player and  GetIsVehicleEngineRunning(vehicle)  then
			if vehicleSignalIndicator == 'off' then
		...
		if IsControlJustPressed(1, Keys[Config.vehicle.keys.signalBoth]) and (has_value(vehiclesCars, vehicleClass) == true) and GetPedInVehicleSeat(vehicle, -1) == player and  GetIsVehicleEngineRunning(vehicle) then
			if vehicleSignalIndicator == 'off' then
