...
window.onload = function () {
		...
		var eventCallback = {
			...
			updateVehicle: function(data) {
				...
				var vehicleKey = document.querySelector('#key');
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						if (vehiclesCars.indexOf(data.type) > -1) {
							...
							vehicleKey.style.display = 'block';
					...
					if ( (data.key.status == true) && (vehicleKey.classList.contains('on') == false) ) {
						vehicleKey.classList.remove('off');
						vehicleKey.classList.add('on');

						eventCallback.sound('sounds/seatbelt-buckle.ogg', { volume: '0.50' });
					}
					else if ( (data.key.status == false) && (vehicleKey.classList.contains('off') == false) ) {
						vehicleKey.classList.remove('on');
						vehicleKey.classList.add('off');
					...
					vehicleEngine.querySelector('span').style.height = data.engine+'%';
					if (data.engine <= 550) {
						if (vehicleEngine.classList.contains('dying') == false) { vehicleEngine.classList.add('dying');	}
					}
